...
def generate_dataset_r_nl_1(seed, n_pat=100):
    np.random.seed(seed)

    x = np.linspace(-2, 2, n_pat)
    y = np.square(x) + np.sin(5 * x) + 0.2 * np.random.randn(len(x))
...
def generate_dataset_blobs(seed, n_pat=100, n_dim=2, std=2.0):
    x, y = make_blobs(
        n_samples=n_pat, n_features=n_dim, cluster_std=std, random_state=seed
    )
    y[y != 1] = -1
    ...
    model = tf.keras.Sequential(
        [
            tf.keras.layers.Conv2D(
                3, filter.shape, kernel_initializer=kernel_init, input_shape=image.shape
            )
        ]
    )
    model.build()
...
def train_rl_model(model, env, gamma=0.99, max_steps=10000, learning_rate=0.01):
    """_summary_
